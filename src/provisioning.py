"""Idempotent Azure resource provisioning for an accelerator scenario.

The provisioner is callable both from the self-hosted FastAPI startup shim and
from the deterministic ``scripts/foundry-provision.py`` command used before a
Hosted Agent starts. Errors propagate to the caller so both paths retain loud
failure semantics.

What it does (idempotent — safe to re-run):

1. **AI Search indexes** — for every ``scenario.retrieval.indexes[]`` entry,
...
# RemoteTool MCP connection. Uses direct REST calls for the preview Knowledge
# Base endpoints because the pinned GA azure-search-documents 12.x SDK covers
# index/vectorizer schemas but does not expose these preview retrieval APIs.
# ---------------------------------------------------------------------------
...
async def _grant_agent_search_access(
    ...
    from azure.mgmt.authorization.aio import (  # pyright: ignore[reportMissingImports]
    ...
    try:
        ...
        try:
            for entry in bundle.retrieval_indexes:
                ...
                if skip_seed:
                    logger.info(
                        "bootstrap.search: skip_seed requested; not seeding %s",
                        entry.name,
                    )
                    continue
...
async def _canary_query(
    ...
    *,
    ...
    With ``enabled=None``, only runs when ``BOOTSTRAP_CANARY=1`` because each
    call costs a model + Search round-trip. An explicit bool overrides the
    environment for deterministic CLI use.
    """
    should_run = os.environ.get("BOOTSTRAP_CANARY") == "1" if enabled is None else enabled
    if not should_run:
        return
    foundry_tool_agents = [
        a for a in bundle.agents
    ...
    try:
        ...
        logger.warning(
            "bootstrap.canary: Foundry project endpoint not set; skipping",
        )
        return
...
async def provision(
    ...
    *,
    ...
    """Provision all scenario resources in their load-bearing order.

    ``skip_seed`` still creates or updates every Search index schema but avoids
    embedding and uploading seed documents. ``canary=None`` preserves the
    ``BOOTSTRAP_CANARY=1`` environment behavior; an explicit bool overrides it.
    Any unrecoverable error propagates to the caller.
    """
    ...
    await _bootstrap_search(bundle, skip_seed=skip_seed)
    await _bootstrap_knowledge(bundle)
    await _bootstrap_foundry(bundle)
    await _canary_query(bundle, enabled=canary)
    logger.info(
