...
| `azd up` fails immediately with `ERROR: failed running pre hooks` | `pre-provision-azd.py` couldn't reach Azure or the manifest is invalid. | Run `python scripts/preflight-deploy.py --region <region>` for a clean error first. |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `azd up` fails inside `Microsoft.CognitiveServices/accounts/deployments` with `InsufficientQuota` or `429` | Default model TPM exceeds region quota. | [Provision step → Model quota / 429](../start/deliver/04-provision-the-customers-azure.md#troubleshooting-azd-up-and-first-boot) |
| `azd up` fails on `Microsoft.CognitiveServices/accounts/projects` | AIServices account not yet fully provisioned, OR region not enrolled for AI Foundry projects. | [Provision step → Foundry project](../start/deliver/04-provision-the-customers-azure.md#troubleshooting-azd-up-and-first-boot) |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `/healthz` returns 503 with `Forbidden` from Cognitive Services or AI Search during `lifespan.startup` | MI role assignments take 1–3 minutes to propagate after `azd up`. | [Provision step → RBAC propagation](../start/deliver/04-provision-the-customers-azure.md#troubleshooting-azd-up-and-first-boot) |
| `/healthz` returns 200 but agent calls fail `ResourceNotFound` for the search index | Bootstrap ran before AI Search RBAC propagated; seed step skipped. | [Provision step → Index not seeded](../start/deliver/04-provision-the-customers-azure.md#troubleshooting-azd-up-and-first-boot) |
| Side-effect tools fail-closed; logs show `hitl.checkpoint -> approver_unreachable` | `HITL_APPROVER_ENDPOINT` not set on the Container App. | [Provision step → HITL approver unreachable](../start/deliver/04-provision-the-customers-azure.md#troubleshooting-azd-up-and-first-boot) |
...
| Workbook auto-deploy fails with `WorkbookAlreadyExists` after a portal-paste install | A pre-existing workbook with the same `name` GUID was created from the legacy paste-install. | Delete the manually-created workbook in the portal (Application Insights → Workbooks → ROI KPIs); re-run `azd provision`. |

## Evals & CI

| Symptom | Cause | Fix |
|---------|-------|-----|
| Evals green locally but red in CI | Env diff: local has `OPENAI_API_KEY` set; CI does not (and shouldn't). | Run `unset OPENAI_API_KEY` locally and re-run; the runner falls back to MI-style auth which matches CI. |
