...
scenario:
  ...
  architecture_diagram:
    path: docs/assets/diagrams/demo-architecture.svg
    provenance: docs/assets/diagrams/demo-architecture.mcp.json
...
def _write_architecture_diagram(context: RepositoryContext) -> None:
    svg = (
        "<svg><text>Generated by Azure Architecture Diagram Builder</text>"
        "<text>Azure Architecture Diagram Builder MCP v1.0.0</text></svg>"
    )
    svg_path = context.root / "docs/assets/diagrams/demo-architecture.svg"
    _write(svg_path, svg)
    provenance = {
        "artifact": {
            "path": svg_path.name,
    ...
    _write(
        context.root / "docs/assets/diagrams/demo-architecture.mcp.json",
...
def test_materialized_scenario_advances_to_provision(tmp_path: pathlib.Path) -> None:
    ...
    _write_architecture_diagram(context)
...
def test_scaffold_rejects_non_mcp_diagram_contract(tmp_path: pathlib.Path) -> None:
    context = _repo(tmp_path)
    brief = "# Solution Brief — Contoso\n\nApproved.\n"
    _write(context.brief_path, brief)
    _approve_architecture(context, brief)
    manifest = yaml.safe_load(context.manifest_path.read_text(encoding="utf-8"))
    manifest["scenario"]["architecture_diagram"]["generator"] = "manual-svg"
    context.manifest_path.write_text(
    ...
    assert state.for_stage(Stage.SCAFFOLD).status == ResultStatus.BLOCKED
    assert any(
        issue.id == "scaffold-architecture-diagram-generator"
        for issue in state.for_stage(Stage.SCAFFOLD).issues
    )
